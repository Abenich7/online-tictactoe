        ...
        #while True:
         #   try:
          #      child_socket.listen(5)  # Allow multiple pending connections
               ...
               # threading.Thread(group=None,target=client_thread_communication,args=[client_socket]).start()
                #child_port_thread=threading.Thread(group=None,target=child_server,args=[port])
                
                #child_port_thread.start()
                #try activating two sockets 
                

                #create a thread when you have a new client. and 
                #every time theres a new message and response to be sent, call the join method to wait for the thread to finish
